...
def sync_customers():
    ...
    for cust_data in customers_to_sync:
        ...
        try:
            ...
            new_customer = Customer(
                ...
                assigned_worker_id=(
                    user.id if user.role == UserRole.FIELD_AGENT else None
                ),
    ...
    response_data = {"msg": "Sync complete", "synced": success_map, "errors": error_map}
                        ...
                        "active_loans": [
                            {
                                ...
                                "next_emi_date": loan.start_date.strftime("%Y-%m-%d"),
                            }
                            for loan in c.loans
...
def get_customer_detail(id):
    ...
    active_loan_obj = next(
        (loan for loan in customer_loans if loan.status == "active"), None
    )
...
def update_customer(id):
    ...
    if "mobile" in data:
        customer.mobile_number = data["mobile"]
    if "address" in data:
    ...
    if "id_proof_number" in data:
        customer.id_proof_number = data["id_proof_number"]
    if "id_proof_type" in data:
        customer.id_proof_type = data["id_proof_type"]
    if "alternate_contact" in data:
        customer.alternate_contact = data["alternate_contact"]
    if "family_head_name" in data:
        customer.family_head_name = data["family_head_name"]
    if "occupation" in data:
        customer.occupation = data["occupation"]
    if "assigned_worker_id" in data:
        customer.assigned_worker_id = data["assigned_worker_id"]
    if "latitude" in data:
        customer.latitude = data["latitude"]
    if "longitude" in data:
        customer.longitude = data["longitude"]
    if "profile_image" in data:
        customer.profile_image = data["profile_image"]
...
def update_customer_status(id):
    ...
    is_valid, message = validate_status_transition(
        customer.status, new_status, user.role.value
    )
    if not is_valid:
    ...
    if name and area:
        similar = (
            Customer.query.filter(
                Customer.name.ilike(f"%{name}%"), Customer.area == area
            )
            .limit(5)
            .all()
        )
...
def get_customer_notes(id):
    ...
    notes = (
        CustomerNote.query.filter_by(customer_id=id)
...
def get_customer_timeline(id):
    ...
    versions = (
        CustomerVersion.query.filter_by(customer_id=id)
        .order_by(CustomerVersion.changed_at.desc())
        .limit(10)
        .all()
    )
    notes = (
...
@customer_bp.route("/<int:id>/share-token", methods=["POST"])
@jwt_required()
def generate_passbook_token(id):
    """Generate a temporary token for client passbook access"""
    customer = Customer.query.get_or_404(id)
    ...
    # Store in database
    pb_token = PassbookToken(
        customer_id=customer.id, token=token, expires_at=expires_at
    )

    db.session.add(pb_token)
    db.session.commit()
        ...
        jsonify(
            {
                ...
                "passbook_url": f"vasool-drive://passbook?token={token}",  # Deep link for the app
            }
        ),
        201,
    )
...
def get_public_passbook(token):
    ...
    if not customer:
        pb_token = PassbookToken.query.filter_by(token=token).first()
        if pb_token:
            if pb_token.expires_at < datetime.utcnow():
                return jsonify({"msg": "Token expired"}), 403
            customer = Customer.query.get(pb_token.customer_id)

    if not customer:
        return jsonify({"msg": "Customer or Token not found"}), 404
