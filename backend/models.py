...
class User(db.Model):
    ...
    face_embeddings = db.relationship(
        "FaceEmbedding", backref="user", cascade="all, delete-orphan"
    )
    qr_codes = db.relationship("QRCode", backref="user", cascade="all, delete-orphan")
    login_logs = db.relationship(
        "LoginLog", backref="user", cascade="all, delete-orphan"
    )
...
class PassbookToken(db.Model):
    __tablename__ = "passbook_tokens"
    id = db.Column(db.Integer, primary_key=True)
...
class Customer(db.Model):
    ...
    )  # 'created', 'verified', 'active', 'inactive', 'closed'
    ...
    worker = db.relationship(
        "User", foreign_keys=[assigned_worker_id], backref="assigned_customers"
    )
...
class Loan(db.Model):
    ...
    status = db.Column(
        db.String(20), default="created"
    )  # 'created', 'approved', 'active', 'closed', 'defaulted'
    is_locked = db.Column(db.Boolean, default=False)
    ...
    customer = db.relationship(
        "Customer", backref=db.backref("loans", cascade="all, delete-orphan")
    )
    emi_schedule = db.relationship(
        "EMISchedule", backref="loan", cascade="all, delete-orphan"
    )
    audit_logs = db.relationship(
        "LoanAuditLog", backref="loan", cascade="all, delete-orphan"
    )
...
class Collection(db.Model):
    ...
    line_id = db.Column(
        db.Integer, db.ForeignKey("lines.id"), nullable=True
...
class Line(db.Model):
    ...
    customers = db.relationship(
        "LineCustomer", backref="line", cascade="all, delete-orphan"
    )
...
class DailySettlement(db.Model):
    ...
    system_cash = db.Column(
        db.Float, default=0.0
    )  # What the system says they collected in CASH
    ...
    difference = db.Column(
        db.Float, default=0.0
    )  # physical + expenses - system (Should be 0)

    notes = db.Column(
        db.Text, nullable=True
    )  # Description of expenses or reasons for shortage
